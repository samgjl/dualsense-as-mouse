...
class ps5ControllerInterface:
    ...
    def __init__(self, resolution = (2560, 1440)):
        ...
        self.knnModel = mm.Model()
        self.knnModel.load_data(directory='./custom_dataset/')
    ...
    def trackpadOff(self, state = None):
        if self.model.isActive and self.model.tracking:
                ...
                # TODO: CURRENT NUMBER BEING RECORDE: 9
                self.micro_iter = 9
                offset = 100

                # self.model.savePNG(self.model.matrix, filename=f"custom_dataset/{self.micro_iter}/{self.micro_iter}_{offset + self.macro_iter}.png") #* Remove in final version?
                print("knn:")
                self.knnModel.knn(self.model.matrix)
                
                
                self.macro_iter = self.macro_iter + 1
                if self.macro_iter == 100:
                    self.stop()
